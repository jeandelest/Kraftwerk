...
public class VariablesMap {
    ...
    public boolean isInQuestionGrid(String questionName){
        return variables.values().stream()
                ...
                .anyMatch(currentVar -> // (FILTER_RESULT variables are upper case)
                        (currentVar.getQuestionItemName().equals(questionName)
                                || currentVar.getQuestionItemName().equalsIgnoreCase(questionName)));
    }

    public Group getQuestionGridGroup(String questionName) {
        return variables.values().stream()
                .filter(Variable::isInQuestionGrid)
                .filter(currentVar -> currentVar.getQuestionItemName().equals(questionName))
                .map(Variable::getGroup)
                .findFirst().orElse(null);
    }
